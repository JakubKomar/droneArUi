...
public class MultipleScreenVLC : MonoBehaviour
{
    ...
    [SerializeField]
    public List<ScreenUserField> canvasScreens;

    [SerializeField]
    public List<Screen> screens2 = new List<Screen>();


    public class ScreenUserField
    {
        [SerializeField]
        public Renderer renderer; //Assign a mesh to render on a 3d object
        [SerializeField]
        public RawImage rawImage; //Assign a Canvas RawImage to render on a GUI object
    ...
    private class Screen
    {
        ...
        public Screen(ScreenUserField screenUserField)
        {
            screen = screenUserField.renderer;
            canvasScreen = screenUserField.rawImage;
            if (screen == null)
            {
                throw new Exception("Tvorba obrazovky - screen == null");
            }

            if (canvasScreen == null)
            {
                throw new Exception("Tvorba obrazovky - canvasScreen == null");
            }
        }
    }
    ...
    void Awake()
    {
        ...
        foreach (ScreenUserField itGmObj in canvasScreens)
        {
            try
            {
                Screen newScreen= new Screen(itGmObj);
    ...
    void Update()
    {
        ...
        foreach(Screen display in screens)
        {
            //Automatically resize output textures if size changes
            var _vlcTexture = display._vlcTexture;
            var texture = display.texture;

            if (_vlcTexture == null || _vlcTexture.width != width || _vlcTexture.height != height)
            {
                ResizeOutputTextures(width, height,display);
            }
    ...
    void ResizeOutputTextures(uint px, uint py,Screen scr)
    {
        var _vlcTexture=scr._vlcTexture;
        var texture=scr.texture;
        var canvasScreen=scr.canvasScreen;
